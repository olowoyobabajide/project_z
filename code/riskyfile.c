...
void suidMemory(unsigned char *data, size_t size, uint32_t mode, char *filename, Report *r){
    ...
    if (mode & S_ISUID) {
        add_finding(r, FINDING_PERMISSION, "SUID Permission Set", "HIGH", "File has SUID bit set, which is extremely suspicious in an APK component.", "SUID", filename, "SUID bit detected");
    }

    if (mode & S_IWOTH){
        add_finding(r, FINDING_PERMISSION, "World Writable File", "HIGH", "File is world-writable, allowing any user to modify it.", "World-Writable", filename, "S_IWOTH bit detected");
    }
}

void hidden_fileMemory(char *filename, Report *r){
    if(fnmatch(".*", basename(filename), 0) == 0){
        add_finding(r, FINDING_PERMISSION, "Hidden File Detected", "LOW", "Hidden file found in the package, which is unusual and may hide malicious content.", "Hidden", filename, basename(filename));
    }
}
